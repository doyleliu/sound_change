...
static void _processFile(SoundTouch *pSoundTouch, const char *inFileName, const char *outFileName)
{
    ...
    while (inFile.eof() == 0)
    {
        ...
        // LOGV("BUFFSamples:%d",BUFF_SIZE);
        // LOGV("numSamples:%d",num);
        put_cnt ++ ;
        // for(int k = 0 ;k < BUFF_SIZE; k++)
        // {
        //     LOGV("putSamples%d: %f",k, sampleBuffer[k]);
        // }
        ...
        do
        {
            ...
            receive_cnt = receive_cnt + nSamples * nChannels ;
            // LOGV("numSamples:%d",nSamples * nChannels);
            // LOGV("numSamples:%d",num);
        } while (nSamples != 0);
    }
    ...
    LOGV("putSamples:%d",put_cnt * 4096);
    LOGV("recieveSamples:%d",receive_cnt);
}
...
extern "C" DLL_PUBLIC jfloat Java_net_surina_soundtouch_SoundTouch_getMyBPM(JNIEnv *env, jobject thiz, jlong handle, jstring inFileName)
{
    // LOGV("[INFO0] BEGIN!!");
    SoundTouch *ptr = (SoundTouch*)handle;
    ...
    try{
        ...
        bpmValue = bpm.getBpm();
        // LOGV("[INFO3]The bmpvlaue is : %f " , bpmValue);
...
extern "C" DLL_PUBLIC jshortArray Java_net_surina_soundtouch_SoundTouch_realtimeprocessFile(JNIEnv *env, jobject thiz, jlong handle, jshortArray buffer, jint buffer_size, jint channels, jshortArray buffer_out, jint voice_num)
{
    LOGV("[INFO0] BEGIN!!");
    SoundTouch *ptr = (SoundTouch*)handle;
    jshort *carr;
    carr = env->GetShortArrayElements(buffer, NULL);
    jshort *carr_out;
    carr_out = env->GetShortArrayElements(buffer_out, NULL);
    int nSamples;
    int nChannels;
    int buffSizeSamples;
    int cnt = 0;//count the receive samples 
    SAMPLETYPE sampleBuffer[buffer_size];

    // jshortArray recieveBuffer;
    // jshort *carr_receive;
    // carr_receive = env->GetShortArrayElements(recieveBuffer, NULL);
    // jshort recieveBuffer[22 * buffer_size];
    

    int sampleRate = 16000;
    
    nChannels = channels;
    LOGV("[INFO1] I am here!");
    ptr->setSampleRate(sampleRate);
    ptr->setChannels(nChannels);

    assert(nChannels > 0);
    // LOGV("[INFO1] I am also here!!");
    for(int i = 0; i < buffer_size; i++)
    {
        // LOGV("Samples%d: %hd",i,carr[i]);
        sampleBuffer[i] = carr[i];
        // LOGV("Samples%d: %hd",i,buffer[i]);
    }
    LOGV("[INFO1] START!!");
    
    buffSizeSamples = buffer_size / nChannels;
    int put_cnt =0 , receive_cnt = 0;
    try{
        // ptr->flush();
        ptr->putSamples(sampleBuffer, buffSizeSamples);
        put_cnt = put_cnt + buffSizeSamples;
        // for(int j = 0; j < buffSizeSamples; j ++)
        // {
        //     s_a[j] = sampleBuffer[j];
        //     // LOGV("putSamples%d: %d",j, s_a[j]);
        // }  
        // LOGV("putSamples: ");
        
        

        // ptr->receiveSamples(sampleBuffer, buffSizeSamples);
        // LOGV("buffSamples:%d",buffSizeSamples);

        // do{
        //     nSamples = ptr->receiveSamples(sampleBuffer, buffSizeSamples);
        //     receive_cnt = receive_cnt + nSamples;
            
        //     for(int j = 0; j < nSamples; j ++)
        //     {
        //         carr_out[voice_num + j] = sampleBuffer[j];
        //         short v_in = carr_out[voice_num + j];
        //         jshort v = (~v_in);
        //         env->SetShortArrayRegion(buffer_out, voice_num + j, 1, &v);
        //         // LOGV("receiveSamples%d: %hd",voice_num + j, carr_out[voice_num + j]);
        //     } 
        //     // for(int j=0; j< buffSizeSamples; j ++) env->SetShortArrayRegion(recieveBuffer,cnt*buffSizeSamples + j, 1, &sampleBuffer[j]);
        //     cnt = cnt + 1;
        //     voice_num = voice_num+ nSamples;
            
        //     // LOGV("recieveSamples:%d",nSamples);
        // }while(nSamples != 0);
        nSamples = ptr->receiveSamples(sampleBuffer, buffSizeSamples);
        receive_cnt = receive_cnt + nSamples;
        ...
        // LOGV("recieveSamples:%d",cnt);
        
        // for(int j = 0; j < 22 * buffSizeSamples; j ++) LOGV("recieveSamples%d: %hd",j, recieveBuffer[j]);
        ...
        LOGV("putSamples:%d",put_cnt);
        LOGV("recieveSamples:%d",receive_cnt);

        env->ReleaseShortArrayElements(buffer, carr, 0);


        // for(int j = voice_num; j < 15 * buffer_size; j ++ ){
        //     carr_out[j] = 0;
        //     short v_in = carr_out[j];
        //     jshort v = (~v_in);
        //     env->SetShortArrayRegion(buffer_out, j, 1, &v);
        // }

        // for(int j = 0; j < buffSizeSamples; j ++)
        // {
        //     carr_out[j] = sampleBuffer[j];
        //     short v_in = carr_out[j];
        //     jshort v = (~v_in);
        //     env->SetShortArrayRegion(buffer_out, j, 1, &v);
        //     // s_b[j] = sampleBuffer[j]; 
        //     // buffer_out[j] = sampleBuffer[j];
        //     // LOGV("recieveSamples%d: %hd",j, carr_out[j]);
        // } 
        env->ReleaseShortArrayElements(buffer_out, carr_out, 0);
        // LOGV("recieveSamples: ");
        // for(int j = 0; j < buffSizeSamples; j ++) LOGV("recieveSamples: %d", s_b[j]); 
    ...
    return buffer_out;
    // env->ReleaseShortArrayElements(recieveBuffer, carr_receive, 0);
    // return recieveBuffer;
    // for(int i=0; i < buffer_size; i ++) buffer_out[i] = sampleBuffer[i]; 
    // return;
